...
-   <div>
    ...
    0.  [Inference of food webs from data (abundances, biological characteristics...): Vagnon et al. 2021 (Ecosphere)](#foodweb_inference)
...
-   <div>
    ...
    2.  [Foodweb analysis: direct and net effects heatmap, identification of trophic chains, calculation of trophic cascades, chain integration](#analysis_section)
...
# install.packages("NetIndices") # to compute trophic levels and omnivory
# install.packages("sparsevar") # for spectralRadius function
# install.packages("expm") # for %^% operator
# install.packages("purrr") # is_empty function
# install.packages("rlist") # for list.reverse()
# install.packages("igraph") # to show food web as a graph
# install.packages("RColorBrewer")
# install.packages("scales")
# install.packages("ggplot2")
# install.packages("docstring") # allow to call the docstring of functions used in this RMarkdown as native R functions, by calling ?<functionName>
...
See <https://doi.org/10.1002/ecs2.3420>, in particular AppendixS4 with the associated github repository <https://github.com/chloevagnon/aNM_method>.
...
# Empirical test lake food web (Vagnon et al. 2023 Ecology Letters)
load("LakesDirectSalmo.RData")
MyFoodWeb <- LakesDirectSalmo[[2]]
diag(MyFoodWeb) <- -1

# Empirical test food web (from Web of Life, Jordi Bascompte's lab)
# load("BascompteFoodWebs.RData")
# MyFoodWeb <- AllFwbs[[2]]
# diag(MyFoodWeb) <- -1
...
#load("StableFwbs_TL4_b.Rdata")
#MyFoodWeb <- StableFwbs_TL4[[1]][["A"]]
...
In the remainder of this document, species in the food web are often referred to by their index in the interaction matrix (their line number). To make it easier for the user to draw parallels between indices and species names, the following code section generates a data-frame associating the index and abbreviated name for each species name.

```{r}
Names <- rownames(MyFoodWeb)
AbbrevNames <- abbreviate(Names, minlength = 5)
NamesAndIndices <- data.frame(Names = Names, Abbreviation = AbbrevNames, Indices = 1:length(Names))
head(NamesAndIndices)
```

## <a name="analysis_section">Food web analysis: direct and net effects, collectivity, connectance, omnivory, trophic cascades, food chain integration</a>

### Heatmaps of direct and net effects

An easily informative representation of the food web is to simply display the interaction matrix as a heatmap: as a reminder, an interaction is read from column $j$ to row $i$.

What's more, displaying the heatmaps of the direct interaction matrix and the net interaction matrix (the inverse matrix) makes it easy to get a general idea of the similarity/dissimilarity of long-term effects to net effects.

```{r}
source("Heatmaps.R")
# docstring(Heatmaps)
Heat <- Heatmaps(MyFoodWeb, NamesAndIndices, Abbrev=TRUE)
Heat[[1]]
Heat[[2]]
```
...
A trophic chain is considered to start from a predator-free species to a prey-free species (basal species). The function below identifies all the chains in the food web meeting this criterion and measures the average chain size in the food web, as well as the standard deviation.

```{r}
source("ChainCount.R")
# doctrsing(ChainCount)
ChainsLength <- ChainCount(MyFoodWeb, NamesAndIndices)
paste("Mean chains length = ", ChainsLength[["MeanChainsLength"]], " sd chains length = ", ChainsLength[["SdChainsLength"]])
```

### In-depth trophic cascades analysis

This section calculates several proxies on the food web and especially on each trophic chains.

However, as can be seen from the previous function on chain length, the network may contain predator apexes with different trophic levels (e.g. 3 and 4), so the user must choose from which trophic level to evaluate the food chains. If there are predator apexes at several levels, the user could execute the function once for each level. On the other hand, if he chooses a trophic level with no predator apex (predator having no predator itself) then the function will return an error message. Furthermore, in order to evaluate the trophic cascade process, the trophic level selected must be at least 3.

All the results are stored in a list of lists called *FoodWebMetrics*. This list contains :
...
-   <div>
    ...
    -   Value of the integration of the chain in the food web, i.e. the ratio of the summed interactions between the chain and the rest of the food web with the summed interactions within the chain. This constitutes a proxy of the collectivity experienced by the trophic chain, the greater if the integration of the chain, the more strongly the chain interacts with the rest of the food web.

    </div>

**NB**: Trophic chains are identified as follows; 1) calculation of the trophic level of each species, 2) identification of the maximum trophic level (must be at least 3) and the species with this level (the *Top*), 3) identification of species with a trophic level two levels below the maximum level (if type="n2"), or basal species (if type="basal), (the *Bottom*), 4) identification of intermediate species link the *Top* and *Bottom* (the *Middle*).

With the parameter type="n2" the analysis considers a trophic chain as running from the top of the food web to the top-2 level, considering a classic trophic cascade as an indirect effect of order 2. Therefore, if a food web has a maximum trophic level greater than 3 trophic cascade (and so trophic chains) will not go all the way down to the basal species.
...
# docstring(FoodWebAnalysis) # to see the docstring
Threshold = 20 # 20% is the threshold used in Ledru et al. 2024
TopLevel = 3
FoodWebMetrics <- FoodWebAnalysis(MyFoodWeb, TopLevel, NamesAndIncides, Threshold, type="basal")
head(FoodWebMetrics)
```
...
```{r}
source("ComputeLinks.R")
# docstring(ComputeLinks)
# Preallocate empty object
WhoWithWho <- vector("list", nrow(MyFoodWeb))
MaxOrder <- 5 # Order of indirect interactions maximum to compute
...
WhoWithWho <- lapply(seq_along(WhoWithWho), function(i) ComputeLinks(i, MyFoodWeb, MaxOrder))
...
# debug(FoodWebGraph)
# docstring(FoodWebGraph) # to see the docstring
# IdxFocusSpecies = the numeric index of the chosen species if any
Graph <- FoodWebGraph(MyFoodWeb, IdxFocusSpecies = 1, Alpha = 0.5)
...
co <- layout_with_fr(Graph) # for Fruchterman-Reingold layout
# co <- NULL # for vertical layout (y coordinate of species = trophic position)

plot(Graph, layout = co, vertex.size = 8, vertex.color = V(Graph)$col,
    edge.width = as.numeric(E(Graph)$weight)*scale,
    edge.curved = 0, edge.color = E(Graph)$col,
    edge.arrow.size = 0.5)

# Add names of species for each node, if any
# Names <- NamesAndIndices$Names # full name
Names <- NamesAndIndices$Abbreviation # Abbreviation 
if (is.null(Names)){
  Names <- seq(1, nrow(MyFoodWeb))
}
text(x = max(par("usr")[1], par("usr")[2]) - 0.5, 
     y = rev(seq(min(par("usr")[3], par("usr")[4]) + 0.1, 
...
# docstring(InvertedChainGraph)
Outputs <- InvertedChainGraph(MyFoodWeb, FoodWebMetrics, Alpha = 0.1)
Graph <- Outputs[["Graph"]]
par(mar = c(1, 1, 1, 1)) # adjust margins
width <- 8  # Ajustez la largeur selon vos besoins
height <- 6  # Ajustez la hauteur selon vos besoins
options(repr.plot.width = width, repr.plot.height = height)
scale <- 0.6 # to adjust edges width

# co <- layout_with_fr(Graph) # for Fruchterman-Reingold layout
co <- NULL # for vertical layout (y coordinate of species = trophic position)

plot(Graph, layout = co, vertex.size = 8, 
    edge.width = as.numeric(E(Graph)$weight)*scale,
    edge.curved = 0, edge.color = E(Graph)$col)
...
if (is.null(Outputs[["InvertedChains"]])){
  print("No chain with cascade inversion")
}
...
# docstring(StabilityAlgorithm)
Value <- StabilityAlgorithm(MyFoodWeb, SelfRegInit = 1, Threshold = 10^-4)
cat("The minimum self-regulation (with the Threshold used) that must be applied to the","\n", "network for it to be stable is : ", Value)
```

### Another proxy of stability: the recovery time

This proxy was introduced by @pimm1977, a larger recovery time indicates a lower stability. However, be careful of some limitations highlighted by @arnoldi2018.

```{r}
RecoveryTime <- -1 / max(Re(eigen(MyFoodWeb, only.values = T)$values))
paste("RecoveryTime is ", RecoveryTime)
```

### Food web dynamics

Food web dynamics can be simulated in a fairly general way using the Generalized Lotka-Volterra (GLV) model. The dynamics of each species depend on its intrinsic growth rate (positive or negative, depending on whether it is autotrophic or heterotrophic) and the sum of interactions with other species (the interaction matrix). The GLV therefore has just one parameter, the growth rate, which we break down into two: the GrowthRate of basal species and the DeathRate of non-basal species.

#### Simulation to equilibrium

The code section below tests whether, given the input parameters (growth rate and death rate), the system actually reaches a stable equilibrium without any extinction. If there are one or more extinctions, the species are removed and the simulation starts again, until a stable system is reached. In the case of a simulated food web, this results in a stable configuration. In the case of an empirically inferred food web, which is suggested to be stable because it has been observed, the user can find the appropriate parameters to obtain a stable system without any extinction or species removal. These parameters can then be used in the next section.
...
Once we have the configuration of the system at equilibrium (the species that persist and their density at equilibrium), we may want to see how the food web responds to the disturbance of one or more species. This allows us, for example, to directly visualize the dynamics of a cascade inversion potentially identified in the dedicated section. This section of the code consists in choosing one or more species to be disturbed, i.e. whose growth/mortality rate will be modified, either positively or negatively. System dynamics in response to this disturbance are then simulated and plotted.

```{r}

source("FoodWebPerturb.R")
# docstring(FoodWebPerturb)
