...
**This section must be run before any other section.**

La première étape nécessaire est le chargement d'un réseau trophique sous la forme d'une matrice d'interaction. C'est une matrice carrée puisque les lignes et les colonnes correspondent aux espèces du réseau. Elles sont rangées dans le même ordre de sorte que la diagonale de la matrice correspond à la self-regulation, l'effet direct d'une espèce sur elle-même. Tandis que tous les autres éléments $a_ij$ de la matrice correspondent à l'effet de l'espèce $j$ (en colonne) sur l'espèce $i$ (en ligne). Les éléments doivent être des numeric. Il doit s'agir d'un food web, donc chaque valeur négative doit avoir une symétrique positive (néanmoins elle peut être différente, typiquement plus faible à cause d'un coefficient de conversion).

Le bloc de code suivant charge un réseau de test, puis la fonction *CheckInit* procède à une vérification du réseau (la matrice d'interaction) qui doit valider les critères précédemment cités sous peine de retourner un message d'erreur.
...
**NB** : L'identification des chaînes trophiques se fait ainsi : 1) calcul du niveau trophique de chaque espèce, 2) identification du niveau trophique maximum (doit valoir au moins 3) et des espèces ayant ce niveau (les *Top*), 3) identification des espèces ayant un niveau trophique de deux crans inférieur au niveau maximum (les *Bottom*), 4) identification des espèces intermédiaire reliant les *Top* et les *Bottom* (les *Middle*). Donc par définition cette analyse considère une chaîne trophique comme partant du sommet du réseau jusqu'au niveau n-2. Dans le cas où un réseau a un niveau trophique maximum supérieur à 3 les chaînes trophiques ne descendront pas jusqu'aux espèces basales.
